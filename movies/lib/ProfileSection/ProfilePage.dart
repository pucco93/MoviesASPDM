            ...
            child: Center(
                child: Text(
                    accountProvider.name == ""
                        ? "Log in to show your account infos and to user the favourites section"
                        : "",
                    style: const TextStyle(fontSize: 20), textAlign: TextAlign.center,))),
        Center(
            child: ClipRRect(child: Container(color: ColorSelect.customSalmon, child: Text(accountProvider.name,
                style: const TextStyle(fontSize: 28))), borderRadius: BorderRadius.circular(10.0))),
        Padding(
            padding: accountProvider.name == ""
                ? const EdgeInsets.all(0)
                : const EdgeInsets.only(top: 20)),
        Center(
            child: ClipRRect(child: Container(color: ColorSelect.customSalmon, child: Text(accountProvider.mail,
                style: const TextStyle(fontSize: 26))), borderRadius: BorderRadius.circular(10.0))),
        Padding(
            padding: accountProvider.name == ""
                ? const EdgeInsets.all(0)
                : const EdgeInsets.only(top: 20)),
        Center(
            child: ClipRRect(child: Container(color: ColorSelect.customSalmon, child: Text(accountProvider.linkedBySocial,
                style: const TextStyle(fontSize: 22))), borderRadius: BorderRadius.circular(10.0))),
        Padding(
            padding: accountProvider.name == ""
                ? const EdgeInsets.all(0)
                : const EdgeInsets.only(top: 20)),
        accountProvider.isLogged
            ? TextButton(child: const Text("Sign out", style: TextStyle(fontSize: 22)), onPressed: _logout,)
            : ElevatedButton(
